...
abstract class HttpApplication Implements IApplication {
	...
	public function processAction($controller, $actionName, HashMap $parameters = null, IRequest $request = null) {
		...
		try {
			...
			/** @var ISettingsLoader*/ 
			$settingsRepository = $this->container->get('yogi\framework\settings\interfaces\ISettingsRepository');
			$controller = $settingsRepository->get('web')['errorHandling']['errorPages']['4xx']['handler'];
			$parameters = new HashMap(array('code' => '404'));
			$controller = $this->container->get('controllers-'.$controller);			
		}
	...
	protected function configControllerPaths() {
		...
		$this->controllerPaths->add('yogi/framework/mvc/controllers/');
	}
