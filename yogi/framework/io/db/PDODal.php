...
/**
	 * 
	 * {@inheritDoc}
	 * @see \yogi\framework\io\db\interfaces\IDal::createTable()
	 */
	public /* boolean */ function createTable($table, array $definition, $dropIfExists = false) {
		
		if($dropIfExists) {
			$statement = $this->pdo->query("DROP TABLE IF EXISTS `".$this->sanitize($table)."`;");
		}
		
		$defString = "(";
		foreach($definition as $def) {
			$defString = $this->sanitize($def->name) . " " . $this->sanitize($def->type) . "(".$this->sanitize($def->length).") ".($def->autoIncrement) ? "AUTO_INCREMENT" : "" ." ". ($def->unique) ? "UNIQUE" : ""; 
		}
		$defString .= ")";
		
		$statement = $this->pdo->prepare("CREATE TABLE ".$this->sanitize($table)." ".$defString);	
		
		return $statement->execute();
	}
	
	public /* void */ function updateTable($table, array $definition) {
		foreach($definintion as $def) {
			if($this->columnExists($table, $def->name)) {
				// alter table alter column
				$this->query("ALTER TABLE ".$this->sanitize($table)." ALTER COLUMN ".$this->getColumnDef($def));
			} else {
				// alter table add column.	
				$this->query("ALTER TABLE ".$this->sanitize($table)." ADD ".$this->getColumnDef($def));
			}
		}
	}
